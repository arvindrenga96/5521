...
def my_cross_val(method: object, matrix: object, target: object, folds: object = 5, diag: object = False, K: object = 2) -> object:
    ...
    for i in range(folds):
        ...
        #print("train",trainx,trainx.size)
        #print("y",train.size)
...
def cross_val(method, X, y, testX, testY,diagonal=False,K=2):
    if method == LinearSVC:
        ...
        #print("inside logistic testY",testY)
        clf = LogisticRegression()
        clf.fit(X, y)
        ypredict = clf.predict(testX)
        #print("printypredict",ypredict)
        length = len(testX)
        #print(length)
        count = 0
        for i in range(length):
    ...
    elif method == "MylogisticReg":
        clf = MylogisticReg2.MyLogisticReg2(13)
        clf.fit(X, y,0.001,1000)
        #print(testX)
        #print(len(testX))
        ypredict = clf.predict(testX)
        #print(ypredict)
